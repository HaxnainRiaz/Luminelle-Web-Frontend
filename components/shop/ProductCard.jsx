...
export default function ProductCard({ product, highlight }) {
  ...
  const renderText = (text) => (highlight ? highlight(text) : text)
  ...
  return (
    <Link href={`/products/${product.slug}`}>
      <Card hover className="h-full">
        ...
        <div className="p-5">
          <h3 className="font-serif text-xl text-primary mb-1">{renderText(product.name)}</h3>
          <p className="text-sm text-neutral-dark mb-3 line-clamp-2">
            {renderText(product.shortDescription)}
          </p>
